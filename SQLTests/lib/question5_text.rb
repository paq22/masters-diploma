...
module Question5Text
  def text()
    "В БД существует отношение people:\\\\
\\begin{tabular}{|#{Question5::REAL_COLUMNS.map{ |k, v| 'c|' }.join}}\n\\hline\n" +
Question5::REAL_COLUMNS.map{ |k, v| "#{k}:#{v}" }.join(' & ') + "\\\\\n\\hline\n" +
(0...Question5::VALUES.values[0].size).to_a.map do |i|
  Question5::VALUES.values.map{ |e| e[i] }.join(' & ')
end.join("\\\\\n\\hline\n") +
"\\\\\n\\hline\n\\end{tabular}
